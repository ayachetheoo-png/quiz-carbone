...
<style>
    ...
    /* ── Métriques : forcer un contraste élevé sur mobile ── */
    ...
    /* ── Adaptations mobile (écrans < 640px) ── */
    @media (max-width: 640px) {
        .quiz-header { padding: 18px 16px; }
        .quiz-header h1 { font-size: 1.4rem; }
        .quiz-header p  { font-size: 0.88rem; }
        .banniere { font-size: 0.92rem; padding: 13px 14px; }
        .conseil  { font-size: 0.88rem; }
...
if calculer:
    ...
    # ── Graphique en barres verticales (plus lisible sur mobile) ─────────────
    categories = ["🚗 Transport", "🏠 Logement", "🍽️ Alim.", "🛒 Conso."]
    valeurs    = [t_transport, t_logement, t_alimentation, t_consommation]
    couleurs   = ["#ef5350", "#42a5f5", "#66bb6a", "#ffa726"]
    ...
    fig.add_trace(go.Bar(
        ...
        text=[f"{v:.1f} t" for v in valeurs],
        textposition="outside",
        cliponaxis=False,
        textfont=dict(size=13, color="#0d0d0d"),
    ))

    # Ligne horizontale : moyenne française
    fig.add_hline(
        y=MOYENNE_FRANCAISE, line_dash="dash", line_color="#1565c0", line_width=2,
    )
    fig.add_annotation(
        x=3, y=MOYENNE_FRANCAISE, xref="x", yref="y",
        text=f"Moy. FR : {MOYENNE_FRANCAISE} t",
        showarrow=False, yshift=12,
        font=dict(color="#1565c0", size=11, family="Arial"),
    )

    # Ligne horizontale : objectif Paris
    fig.add_hline(
        y=OBJECTIF_PARIS, line_dash="dash", line_color="#2e7d32", line_width=2,
    )
    fig.add_annotation(
        x=0, y=OBJECTIF_PARIS, xref="x", yref="y",
        text=f"Objectif : {OBJECTIF_PARIS} t",
        showarrow=False, yshift=12,
        font=dict(color="#2e7d32", size=11, family="Arial"),
    )

    y_max = max(max(valeurs) * 1.45, MOYENNE_FRANCAISE * 1.3, 13.0)
    fig.update_layout(
        title=dict(
            text=f"Total : {total:.1f} tCO₂e/an",
            font=dict(size=15, color="#0d0d0d"),
        ),
        yaxis=dict(
            title=dict(text="tCO₂e/an", font=dict(size=12, color="#0d0d0d")),
            range=[0, y_max],
            tickfont=dict(size=12, color="#0d0d0d"),
        ),
        xaxis=dict(tickfont=dict(size=13, color="#0d0d0d")),
        height=400,
